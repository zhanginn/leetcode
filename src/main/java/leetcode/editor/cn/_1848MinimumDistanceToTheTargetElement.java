...
public class _1848MinimumDistanceToTheTargetElement {
    public static void main(String[] args) {
        Solution solution = new _1848MinimumDistanceToTheTargetElement().new Solution();
    ...
    class Solution {
        public int getMinDistance(int[] nums, int target, int start) {
            int resultAbs = nums.length;
            for (int i = 0; i < nums.length; i++) {
                if (nums[i] == target) {
                    int tempAbs = Math.abs(i - start);
                    resultAbs = Math.min(tempAbs, resultAbs);
                }
            }
            return resultAbs;
        }
